...
- 📐 **[可控构图与多平台导出](docs/platform-framing.md)**：固定成片版本，选择模糊背景、黑边留白或满屏裁切，微调裁切位置并预览真实画面；勾选目标平台依次导出，支持取消与失败重试，网页 / CLI / MCP 共用本地能力。
...
- 🔔 **全局任务中心**：跨项目查看自动出片、合成、云端任务和全部活跃批次；空闲时继续发现新任务，回到页面或展开面板立即更新，网络失败时保留上次状态并支持重试。失败与中断提醒持续保留到新一轮接替，点击可直达恢复入口。模型目录随服务地址和密钥切换即时重置，避免旧列表或迟到响应干扰选择。详见 [任务状态与模型选择](docs/task-recovery.md)。
...
| **多平台导出** | 固定成片版本，预览构图后按所选平台导出；支持完整保留或指定位置裁切，附实测码率报告，原片不覆盖 |
...
**已完成**
- ✅ **出片主链路**：AI 脚本（5 品类 × 十风格四形态 + 黄金 3 秒 + 平台 SEO）→ 商品保真素材（7 平台 30+ 模型）→ i2v 动态镜头（运镜引擎 / 命名运镜预设每镜可选 / 画面风格 Look / 链式首尾帧 / 强度三档 / 逐镜重跑）→ FFmpeg 合成（爆款字幕 / 免费多音色配音 / BGM / 智能转场 / 风格配方 / 质量预设）→ 多平台导出（构图预览与码率预算）
