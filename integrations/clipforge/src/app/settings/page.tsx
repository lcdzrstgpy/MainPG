...
import { LuUpload, LuPalette, LuTriangleAlert } from "react-icons/lu";
...
const MEDIA_PROVIDER_KEYS = ["volcengine", "suchuang"] as const;
...
const AI_PROVIDERS = [
  ...
  },
  {
    key: "suchuang",
    name: "速创",
    descKey: "providerSuchuangDesc",
    tipKey: "providerSuchuangTip",
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M4 7.5 12 3l8 4.5v9L12 21l-8-4.5z" />
        <path d="m8 10 4 2.25L16 10M12 12.25V17" />
      </svg>
    ),
...
export default function SettingsPage() {
  ...
  const testTTS = async () => {
    ...
    try {
      ...
      setTtsTestError(error instanceof Error ? error.message : "试听失败");
    }
  };
  ...
  const onChangeTTSProvider = (provider: TTSProvider) => {
    ...
    setTTS({
      ...
      voice: meta.defaultVoice,
      ...(meta.value === "volcengine-speech" && { resourceId: meta.defaultResourceId, speechRate: 10 }),
    });
  };
  ...
  const enabledProviders = Object.entries(providers)
    .filter(([name, p]) => MEDIA_PROVIDER_KEYS.includes(name as typeof MEDIA_PROVIDER_KEYS[number]) && p.enabled && p.apiKey)
        ...
        <div className="md:flex md:items-start md:gap-8">
          ...
          <section className="min-w-0 flex-1">
            <div className="space-y-4">
              {AI_PROVIDERS.filter((platform) => MEDIA_PROVIDER_KEYS.includes(platform.key as typeof MEDIA_PROVIDER_KEYS[number])).map((platform) => {
                ...
                <CardContent className="p-5">
                  ...
                  {tts.enabled && (
                    <div className="space-y-4">
                      ...
                      {ttsMeta.value === "volcengine-speech" ? (
                        <>
                          <div className="space-y-1.5">
                            <Label className="text-xs text-muted-foreground">{t("apiKeyLabel")}</Label>
                            <PasswordInput value={tts.apiKey} onChange={(apiKey) => setTTS({ ...tts, apiKey })} placeholder="火山语音控制台 API Key" />
                          </div>
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                              <Label className="text-xs text-muted-foreground">资源 ID</Label>
                              <Input value={tts.resourceId ?? ttsMeta.defaultResourceId ?? "seed-tts-2.0"} onChange={(e) => setTTS({ ...tts, resourceId: e.target.value })} placeholder="seed-tts-2.0" className="font-mono text-xs" />
                            </div>
                            <div className="space-y-1.5">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                              <Label className="text-xs text-muted-foreground">音色 ID（须为已授权的 Seed TTS 2.0 音色）</Label>
                              <Input value={tts.voice} onChange={(e) => setTTS({ ...tts, voice: e.target.value })} placeholder="从火山语音控制台复制音色 ID" className="font-mono text-xs" />
                            </div>
                            <div className="space-y-1.5">
                              <Label className="text-xs text-muted-foreground">语速（-50 到 100，推荐 10）</Label>
                              <Input type="number" min={-50} max={100} step={1} value={tts.speechRate ?? 10} onChange={(e) => setTTS({ ...tts, speechRate: Number(e.target.value) })} className="font-mono text-xs" />
                            </div>
                          </div>
                          <p className="text-[11px] text-muted-foreground/80">接口固定为 openspeech.bytedance.com 的火山语音 V3 HTTP 接口；不用填写 Ark baseUrl。</p>
                        </>
                      ...
                      <div className="pt-3 mt-1 border-t border-border/50">
                        ...
                        {ttsTestError && <p className="mt-2 text-[11px] text-destructive break-words">{ttsTestError}</p>}
                      </div>
                    </div>
                  )}
                </CardContent>
