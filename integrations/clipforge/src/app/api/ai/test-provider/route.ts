...
const DEFAULT_BASE: Record<string, string> = {
  ...
  suchuang: "https://api.wuyinkeji.com",
...
function buildProbe(name: string, apiKey: string, baseUrl?: string): Probe {
  ...
  if (name === "suchuang") {
    // There is no free /models endpoint. A synthetic task lookup never creates
    // work; 401/403 remain an invalid key, while any other response proves that
    // the request reached the authenticated API surface.
    return {
      url: `${base}/api/async/detail?id=key-probe&key=${encodeURIComponent(apiKey)}`,
