 ...
 *
 * Supports paid TTS providers, dispatched by config.provider.
 * - volcengine-speech: Volcengine Speech V3 HTTP Chunked, synchronous MP3 collection.
 * - openai: legacy OpenAI-compatible /audio/speech, synchronous mp3.
...
export interface TTSConfig {
  ...
  groupId?: string;
  /** Volcengine Speech V3 resource id, for example seed-tts-2.0. */
  resourceId?: string;
  /** Volcengine Speech V3 speech rate in the official -50..100 range. */
  speechRate?: number;
  /**
...
export async function generateSpeech(text: string, config: TTSConfig): Promise<Buffer> {
  ...
  const cacheKey = ttsCacheKey({
    ...
    ...(config.speechRate != null && { speechRate: config.speechRate }),
    text: clean,
  });
...
function dispatchTTS(clean: string, config: TTSConfig): Promise<Buffer> {
  switch (config.provider) {
    ...
    case "volcengine-speech":
      return generateSpeechVolcengineV3(clean, config);
    default:
...
/** Parse the JSON/SSE records returned by the V3 HTTP Chunked endpoint into MP3 bytes. */
export async function parseVolcengineTTSStream(chunks: AsyncIterable<Uint8Array>): Promise<Buffer> {
  ...
  const consume = (raw: string) => {
    ...
    if (!json) return;
    let packet: VolcengineTTSChunk;
    try {
      packet = JSON.parse(json) as VolcengineTTSChunk;
    } catch {
      throw new Error("火山语音 V3 返回了无法解析的数据帧");
    }
    const code = packet.code ?? 0;
    if (code !== 0 && code !== 20000000) {
      throw new Error(`火山语音 V3 请求失败: ${code}${packet.message ? ` - ${clipErr(packet.message)}` : ""}`);
    }
    if (packet.data) audio.push(Buffer.from(packet.data, "base64"));
  ...
  if (!completed) throw new Error("火山语音 V3 未返回完成状态");
  const merged = Buffer.concat(audio);
  if (!merged.length) throw new Error("火山语音 V3 未返回音频数据");
  return merged;
}
...
async function generateSpeechVolcengineV3(text: string, config: TTSConfig): Promise<Buffer> {
  const resourceId = config.resourceId || "seed-tts-2.0";
  const speechRate = clamp(Number(config.speechRate ?? 0), -50, 100);
  const resp = await fetch(config.baseUrl || "https://openspeech.bytedance.com/api/v3/tts/unidirectional", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    ...
    body: JSON.stringify({
      ...
      req_params: {
        ...
        model: config.model || "seed-tts-2.0-expressive",
        audio_params: {
          format: "mp3",
          sample_rate: 24000,
  ...
  if (!resp.ok) {
    const errText = await resp.text().catch(() => "");
    throw new Error(`火山语音 V3 请求失败: ${resp.status} ${resp.statusText}${errText ? ` - ${clipErr(errText)}` : ""}`);
  }
  if (!resp.body) throw new Error("火山语音 V3 未返回响应流");
  return parseVolcengineTTSStream(responseChunks(resp.body));
}
