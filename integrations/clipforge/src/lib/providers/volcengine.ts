...
/** Seedream generation is synchronous and can exceed the shared 30s HTTP default at peak load. */
const VOLCENGINE_IMAGE_TIMEOUT_MS = 120_000
...
export class VolcEngineProvider extends BaseProvider {
  ...
  async generateImage(options: ImageOptions): Promise<ImageResult> {
    ...
    const resp = await this.request<ArkImageResponse>('/images/generations', {
      method: 'POST',
      body,
      timeout: VOLCENGINE_IMAGE_TIMEOUT_MS,
    })
