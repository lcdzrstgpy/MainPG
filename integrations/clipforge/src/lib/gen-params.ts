...
export const MAINPG_MEDIA_PROVIDER_NAMES = ["volcengine", "suchuang"] as const;

type MediaProviderSetting = { enabled?: boolean; apiKey?: string; baseUrl?: string };

/**
 * Keep every generation page on the same two-platform boundary. Persisted
 * settings from upstream ClipForge may retain old keys, but they must not be
 * forwarded to the media API after the MainPG integration.
 */
export function enabledMainPgMediaProviders(providers: Record<string, MediaProviderSetting>) {
  return Object.entries(providers)
    .filter(([name, setting]) => MAINPG_MEDIA_PROVIDER_NAMES.includes(name as typeof MAINPG_MEDIA_PROVIDER_NAMES[number]) && setting.enabled && setting.apiKey)
    .map(([name, setting]) => ({ name, apiKey: setting.apiKey!, baseUrl: setting.baseUrl }));
}
