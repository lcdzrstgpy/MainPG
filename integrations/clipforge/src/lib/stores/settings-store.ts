...
export interface TTSSetting {
  ...
  groupId?: string;
  /** 火山语音 V3 资源 ID，默认 seed-tts-2.0 */
  resourceId?: string;
  /** 火山语音 V3 语速，范围 -50 到 100 */
  speechRate?: number;
}
...
export function migrateSettings(state: SettingsState): SettingsState {
  ...
  // v6: the former "火山方舟" preset sent OpenAI /audio/speech payloads to an Ark URL.
  // Speech V3 uses a different endpoint and credential type, therefore never carry a media/Ark
  // key or legacy speaker id over as if it were a valid Speech API configuration.
  if (state?.tts && state.tts.provider !== "volcengine-speech") {
    state.tts = {
      ...state.tts,
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    ...
    }),
    {
      ...
      // v6：旧的方舟 /audio/speech 配音配置改为火山语音 V3，且不复用旧媒体 Key。
      version: 6,
