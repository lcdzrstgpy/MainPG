...
- **[Framing and platform export](docs/platform-framing.md)**: pin a finished version, preview blur / black padding / positioned crop, then export to selected destinations. Local processing with cancellation and per-platform retry, shared by Web / CLI / MCP.
...
- 🔔 **Global task center**: monitor pipelines, renders, cloud tasks and every active batch across projects. Idle polling discovers new work; returning to the page or opening the panel refreshes immediately. Failed refreshes retain the last known status with a retry action. Failed and interrupted runs remain visible until a newer run replaces them. Model lists reset when the endpoint or key changes, and late responses cannot overwrite the current selection list.
