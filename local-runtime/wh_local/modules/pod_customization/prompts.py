...
def _detect_theme(business_fields: Mapping[str, object] | None, free_text: str = "") -> str | None:
    ...
    the free-text base prompt, so an explicit theme always wins over incidental
    words in the product description or creative prompt.
    """
    theme, keywords = _brief_theme_keywords(business_fields)
    structured = f"{theme} {' '.join(keywords)}"
    for source in (structured, free_text):
...
def _motif_for_style(
    business_fields: Mapping[str, object] | None, style_index: int, free_text: str = ""
) -> str:
    """Pick a deterministic motif that stays on the brief's theme.

    Resolution order: declared theme (structured fields or free text) ->
    literal design-theme text -> style keywords -> generic fallback. The
    generic pool is only reached when the user supplied no theme anywhere, so
    it can never override a theme.
    """
    theme = _detect_theme(business_fields, free_text)
    if theme:
        motifs = _THEME_MOTIFS[theme]
        return motifs[(style_index - 1) % len(motifs)]
    theme_text, keywords = _brief_theme_keywords(business_fields)
    if theme_text:
        return f"the literal '{theme_text}' theme"
    if keywords:
        return f"subjects derived from the style keywords ({', '.join(keywords[:3])})"
...
def build_style_listing_prompt(
    ...
    *,
    ...
    subject. The base prompt is also scanned as free text so a theme written
    only in the creative prompt is still detected.
    """
    if style_index < 1:
    ...
    motif = _motif_for_style(business_fields, style_index, base_prompt)
    ...
    rules = [
        ...
        "Use the motif, composition, palette, rendering, and density recipe as diversity guidance while staying strictly within the brief's theme. Do not fall back to the template artwork or a generic design used for another style.",
