...
from ....shared.miaoshou_workbook import (
    ...
    MS_TEMPLATE_FILES,
    miaoshou_row_values,
    write_miaoshou_workbook,
)


# 店小秘导入默认值（对齐原型 native_product_engine 常量）；发货时效默认值见共享模块。
...
# 从第 3 行开始逐 SKU 写入。
#
# 模板缓存、列映射与逐行取值已抽到 wh_local/shared/miaoshou_workbook.py，与 POD 定制
# 共用同一份口径；这里只保留「商品结果 -> 店小秘行 -> 妙手行」的产品处理侧适配。
...
def create_miaoshou_workbook(
    ...
    *,
    ...
    expanded: list[dict[int, Any]] = []
    for product_row in rows:
        expanded.extend(_miaoshou_row_values(product_row, kind))
    return write_miaoshou_workbook(expanded, kind, destination, template_dir=template_dir)


def _miaoshou_row_values(row: dict[str, Any], kind: str) -> list[dict[int, Any]]:
    """把一个商品的导出结果展开为若干妙手 SKU 行（每行=一个变种，无变种=单行）。

    规格名/值、申报价、建议售价、长宽高、图片等基础取值与店小秘导出行保持同口径
    （复用 _dxm_single_export_row）；妙手侧差异统一在共享模块处理：
    ...
    for variant in targets:
        ...
        if variant_key in seen:
            continue
        seen.add(variant_key)
        # SKU 规格图：店小秘 18/19 列为商品级轮播/素材图，8 列为 SKU 规格图。妙手把
        # SKU 规格图落到颜色图（服饰）/预览图（非服饰），缺失或「全部使用主图替代」时
        # 回退商品级素材图（19 列，商品级，避免沿用已被换成规格图的 8 列）。
        ...
        exported.append(
            miaoshou_row_values(
                ...
                stock=_normalize_stock(dxm_row[24]),
                weight_g=_miaoshou_raw_weight(row, variant, preview_overrides, core_fields),
