...
def miaoshou_row_values(
    ...
    *,
    ...
    - 图片：产品处理侧传已解析的 ``sku_image``（SKU 规格图，落到颜色图/预览图）与
      ``product_image``（商品级素材图）；两者都未传（POD 定制）时沿用店小秘预览图列；
    ...
    if sku_image is None and product_image is None:
        # POD 定制：沿用店小秘预览图列作为商品级主图。
        material_image = str(dxm_row[8] or "").strip()
        if not material_image and carousel_text:
            material_image = carousel_text.splitlines()[0]
        if kind == MS_KIND_APPAREL:
            values[columns["color_images"]] = carousel_text
        else:
            values[columns["carousel_images"]] = carousel_text
            values[columns["preview_image"]] = material_image
    else:
        # 产品处理：SKU 规格图落到颜色图/预览图；商品级素材图保持商品级不变。
        material_image = str(product_image or "").strip()
        if not material_image:
            material_image = carousel_text.splitlines()[0] if carousel_text else ""
        resolved_sku_image = str(sku_image or "").strip()
        if kind == MS_KIND_APPAREL:
            values[columns["color_images"]] = resolved_sku_image or carousel_text
        else:
            values[columns["carousel_images"]] = carousel_text
            values[columns["preview_image"]] = resolved_sku_image
    values[columns["material_image"]] = material_image
