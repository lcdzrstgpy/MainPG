...
4. 按用途点击“只发布更新（不同步官网）”“上传并发布内测版”或“上传并发布公共版”。

后台先验证 EXE 文件头和版本递增，再读取 EXE 尾部的补丁描述和变更文件，逐个核对路径、体积与 SHA-256。随后把完整安装包提交到 EV Sign。只有签名后的文件通过 Authenticode 校验，后台才会签署完整更新清单和补丁清单并原子发布。选择内测版或公共版时，还会把同一份已签名文件原子同步到对应的官网固定下载地址；选择“只发布更新”时，官网两个固定下载文件保持不变。客户端版本与补丁起点不一致或补丁生成失败时，仍使用完整安装包。每个安装包只长期保存一份，临时补丁文件在请求结束后删除。

发布页面会显示安装包上传字节进度，并把 EV Sign、Authenticode 验证、增量补丁、官网文件发布和清单签名等阶段持久化到数据库。上传完成后即使刷新或重新登录，页面也会继续显示服务器端任务进度；服务进程重启时，未完成任务会明确标记为中断，不会长期停留在“处理中”。

## 生产部署注意

- `UPDATE_PUBLISH_DIR` 应指向官网 `/mainpg/windows` 对应的静态目录，Nginx 只读提供下载。
- `UPDATE_INTERNAL_DOWNLOAD_PATH` 应指向官网内测版按钮对应的固定文件；`UPDATE_PUBLIC_DOWNLOAD_PATH` 应指向公共版按钮对应的固定文件。服务账户必须能写入这两个文件所在目录。
