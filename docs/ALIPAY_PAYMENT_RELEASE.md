...
- 固定套餐为 49 / 99 / 499 / 999 元，分别赠送基础积分的 25% / 50% / 75% / 100%；自定义金额仍按原价到账，不参与赠送。
- 新建订单创建时保存基础、赠送与合计积分的不可变快照。历史翻倍活动订单继续按其原快照结算；已支付订单不补发、不扣回。
...
1. 账号服务使用包含 `local-runtime/wh_local/customer/alipay_gateway.py`、`auth_server.py` 与 `billing.py` 的版本，并重启 `wh-customer-auth.service`。服务初始化会关闭旧的 `topup_double` 开关；该记录仅保留作历史订单审计，新订单不读取它。
...
1. 本次最新构建的 `web-frontend/dist`，其中必须包含个人中心的支付宝选择、49 / 99 / 499 / 999 元固定套餐、25%~100% 阶梯赠送说明、自定义金额原价说明、创建订单、跳转和回跳刷新逻辑。
...
2. 选择支付宝，确认固定套餐仅有 49 / 99 / 499 / 999 元，且到账积分分别为 6,125 / 14,850 / 87,325 / 199,800；微信按钮应不可创建订单。
3. 验证一笔 `1` 至 `3000` 元整数自定义金额订单，确认 `promotion_bonus_points` 为 0、合计等于基础积分。
4. 使用支付宝沙箱或经批准的小额真实支付完成一笔固定套餐订单；到账账本应包含基础充值流水与独立 `topup_promotion_bonus` 流水。
5. 支付后应回到本机个人中心；刷新后“可用积分”和“最近订单”均显示服务端的新结果和订单快照。
6. 重复刷新、重复打开回跳页或重复通知不得再次增加积分。
7. 如有历史 4999 元翻倍 pending 订单，验证其支付结算仍使用已保存的翻倍快照，而不是当前 999 元套餐规则。
