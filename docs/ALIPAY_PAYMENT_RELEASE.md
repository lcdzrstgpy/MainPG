...
- 固定套餐为 49 / 99 / 499 / 4999 元；每档常驻赠送基础积分的 25%。自定义金额仍按原价到账，不参与赠送。
...
1. 本次最新构建的 `web-frontend/dist`，其中必须包含个人中心的支付宝选择、49 / 99 / 499 / 4999 元固定套餐、25% 赠送说明、自定义金额原价说明、创建订单、跳转和回跳刷新逻辑。
...
2. 选择支付宝，确认固定套餐仅有 49 / 99 / 499 / 4999 元，且到账积分分别为 6,125 / 12,375 / 62,375 / 624,875；微信按钮应不可创建订单。
...
7. 如有翻倍活动期间创建的历史 pending 订单，验证其支付结算仍使用已保存的翻倍快照，而不是常驻 25% 规则。
