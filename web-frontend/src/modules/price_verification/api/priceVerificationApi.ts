...
export const priceVerificationApi = {
  ...
  selectBatchSourceCandidate: (batchId: string, skcId: string, candidate: SourceCandidate, priceCny?: string) => httpJson<BatchSourcingState>(`${base}/capture-batches/${encodeURIComponent(batchId)}/sourcing/candidates`, { method: "POST", body: { skc_id: skcId, candidate, ...(priceCny !== undefined ? { price_cny: priceCny } : {}) } }),
