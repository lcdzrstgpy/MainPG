...
export function LinkedSourcePanel({ links, onUnlink }: Props) {
  ...
  return <section className="price-verification-panel price-verification-linked-panel"><div className="price-verification-panel-heading"><div><p className="eyebrow">STEP 04 · SOURCED</p><h2>已关联 1688 货源</h2><p>集中展示所有已保留 SKC 关联的 1688 代发货源：SKC 标题、站点、调整后申报价一目了然，每条 1688 链接都按“候选源价（可调）”核算了利润（净利润 / 利润率 / 达标与否）。数据已入库，刷新页面不会丢失。</p></div><div className="price-verification-source-empty"><div><span>已关联 SKC</span><strong>{groups.length}</strong></div><div><span>已关联 1688 链接</span><strong>{links.length}</strong></div></div></div>
    {groups.length ? <div className="price-verification-linked-groups">{groups.map(([skcId, groupLinks]) => {
      const first = groupLinks[0];
      return <div className="price-verification-linked-group" key={skcId}><div className="price-verification-source-group-head"><strong>{skcId} · {first?.product_title || "未命名商品"}</strong><small>{siteLabel(first?.site)} · 调整后申报价 {moneyText(first?.selling_price)} · 已关联 {groupLinks.length} 条 · 价格与利润已按候选源价（可调）校准</small></div><div className="price-verification-source-linked">{groupLinks.map((link) => <div key={link.id}><a href={link.source_url} target="_blank" rel="noreferrer"><img src={link.main_image_url} alt="" loading="lazy" /><span><b className="source-link-title">{link.source_title || link.offer_id}</b><small className="source-link-offer">offer {link.offer_id}</small></span></a><span className="source-link-source-price"><small>源价</small><b>¥{link.price_cny ?? "—"}</b></span><LinkProfit profit={link.profit ?? null} /><button onClick={() => void unlink(link)} disabled={busyId === link.id}>{busyId === link.id ? "解除中…" : "解除"}</button></div>)}</div></div>;
