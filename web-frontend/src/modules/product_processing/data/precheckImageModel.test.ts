...
const base = {
  ...
  detail_asset_ids: ["d"],
  semantic_asset_ids: { overview: "a", detail_hero: "d" },
};
...
test("reorder and select-main are identity based", () => {
  ...
  assert.deepEqual(moved.semantic_asset_ids, base.semantic_asset_ids);
  const selected = selectMainAsset(base, "c");
  assert.equal(selected.main_asset_id, "c");
  assert.deepEqual(selected.carousel_asset_ids, base.carousel_asset_ids);
  assert.deepEqual(selected.semantic_asset_ids, base.semantic_asset_ids);
});

test("selecting an asset outside carousel inserts it exactly once", () => {
  const selected = selectMainAsset(base, "x");
  assert.equal(selected.main_asset_id, "x");
  assert.deepEqual(selected.carousel_asset_ids, ["a", "b", "c", "x"]);
...
test("multi-file additions preserve selection order and ignore only new duplicates", () => {
...
test("deleting the final carousel image leaves a visible no-main state", () => {
  const result = removeAsset(
    { main_asset_id: "a", carousel_asset_ids: ["a"], detail_asset_ids: [], semantic_asset_ids: {} },
    "carousel",
    "a",
  );
