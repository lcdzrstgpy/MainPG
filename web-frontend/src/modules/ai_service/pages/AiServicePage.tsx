    ...
    <section className="ai-service-page" aria-label="AI 服务">
      ...
      <main className="ai-service-workspace">
        ...
        <div className="ai-mode-tabs" role="tablist" aria-label="创作模式">
          {(["chat", "generate", "edit"] as AiCreationMode[]).map((item) => <button key={item} type="button" className={mode === item ? "is-active" : ""} onClick={() => void handleModeChange(item)} role="tab" aria-selected={mode === item}><span className={`iconfont ${modeIcon(item)}`} aria-hidden="true" /> {modeLabels[item]}</button>)}
        </div>
        ...
        <div className="ai-composer-wrap">
          ...
          <div className="ai-composer"><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} onPaste={handleChatPaste} onKeyDown={handleChatKeyDown} placeholder={mode === "chat" ? "输入问题；可附图片、资料文件或开启联网搜索…" : mode === "edit" ? "上传商品图，并描述想替换的背景或场景…" : "描述你想要创作的商品视觉…"} /><div className="ai-composer-actions"><div>{(mode !== "chat" || selectedModel.acceptsImageInput) && <><input ref={fileInputRef} type="file" accept="image/*" hidden onChange={(event) => void selectImage(event.target.files?.[0])} /><button type="button" className="ai-attach-button" onClick={() => fileInputRef.current?.click()}><span className="iconfont icon-image" /> 图片</button></>}{mode === "chat" && <><input ref={documentInputRef} type="file" accept=".txt,.csv,.xlsx,.docx" hidden onChange={(event) => void selectDocument(event.target.files?.[0])} /><button type="button" className="ai-attach-button" onClick={() => documentInputRef.current?.click()}><span className="iconfont icon-file" /> 文件</button><button type="button" className={`ai-search-toggle ${webSearchEnabled ? "is-on" : ""}`} onClick={() => setWebSearchEnabled((value) => !value)} title="联网搜索公开资料"><span className="iconfont icon-search" />联网搜索</button></>}<span className="ai-upload-hint">{mode === "chat" ? selectedModel.acceptsImageInput ? "图片与资料文件可同时附上" : "当前模型仅支持文字与资料文件" : "上传商品图后开始创作"}</span></div><button type="button" className="ai-generate-button" disabled={!prompt.trim() || isGenerating || isUploadingImage || apiStatus !== "ready"} onClick={() => void generate()}>{mode === "chat" ? "发送" : "开始创作"}<span className="iconfont icon-arrowright" /></button></div></div>
        </div>
...
function GeneratedAssetCards({ imageUrls, groups }: { imageUrls?: string[]; groups?: Array<{ label: string; imageUrls: string[] }> }) {
  if (groups) return <div className="ai-pod-output-groups" aria-label="分组生成图">{groups.map((group) => <section key={group.label}><h3>{group.label}</h3><GeneratedAssetCards imageUrls={group.imageUrls} /></section>)}</div>;
