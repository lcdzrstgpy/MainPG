...
export function PodBatchGallery({ batch, busyAction, onOpenResult, onRegenerateStyle, onRegenerateTitle, onRetryFailed, onExportDianxiaomi }: Props) {
  ...
  if (!batch) return <section className="pod-gallery pod-gallery-empty" aria-label="POD 批次画廊"><span className="iconfont icon-skin" aria-hidden="true" /><h2>从一个模板开始本批次</h2><p>每一款只发起一次生成请求，返回的图片会固定归在同一款下。</p></section>;
  ...
  return <><section className="pod-gallery" aria-label="POD 批次画廊">
    <header className="pod-gallery-header">
      <div><h2>{batch.title || `${batch.template_name} 创作批次`}</h2><p>当前批次 <b>{batch.count} 款</b> · 每款一次生成 · {batch.template_name}</p></div>
    ...
    <div className="pod-gallery-progress"><div role="progressbar" aria-label="POD 批次进度" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress}><span style={{ width: `${progress}%` }} /></div><small>完成 {batch.completed_count} 款 · 失败 {batch.failed_count} 款 · {progress}%</small></div>
