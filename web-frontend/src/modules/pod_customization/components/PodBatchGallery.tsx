...
export function PodBatchGallery({ batch, busyAction, onOpenResult, onRegenerateStyle, onRegenerateTitle, onExportDianxiaomi }: Props) {
  ...
  if (!batch) return <section className="pod-gallery pod-gallery-empty" aria-label="POD 批次画廊"><span className="iconfont icon-skin" aria-hidden="true" /><h2>从一个模板开始本批次</h2><p>每一款只发起一次四宫格生成请求，返回的四张图会固定归在同一款下。</p></section>;
  ...
  return <><section className="pod-gallery" aria-label="POD 批次画廊">
    <header className="pod-gallery-header">
      <div><span>POD BATCH · {batch.id.slice(0, 8)}</span><h2>{batch.title || `${batch.template_name} 创作批次`}</h2><p>当前批次 <b>{batch.count} 款</b> · 每款一次四宫格请求 · {batch.template_name}</p></div>
