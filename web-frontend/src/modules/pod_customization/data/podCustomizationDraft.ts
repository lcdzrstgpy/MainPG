...
export const POD_CUSTOMIZATION_DRAFT_VERSION = 1;
...
export function loadPodCustomizationDraft(
  ...
  try {
    ...
    if (!isPodCustomizationDraft(parsed)) throw new Error("invalid payload");
    return { state: cloneDraft(parsed) };
  } catch {
    try {
...
function cloneDraft(state: PodCustomizationDraft): PodCustomizationDraft {
  return {
    ...
    listing_fields: {
      ...
      sku_names: Array.isArray(state.listing_fields.sku_names) ? [...state.listing_fields.sku_names] : [],
    },
