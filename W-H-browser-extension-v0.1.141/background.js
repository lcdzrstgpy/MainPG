...
let reportedConnectionStatus = null;
...
// 插件必须把真实连接状态写进 storage，popup 才能如实显示。
// 旧实现里 popup 只按 connectionContext 是否存在渲染“已连接”，
// 于是轮询已被工作台拒绝、会话早已失效时，面板仍显示“已连接”。
async function recordConnectionStatus(state, detail = "") {
  const next = { state, detail: String(detail || "").slice(0, 240), at: Date.now() };
  if (
    reportedConnectionStatus
    && reportedConnectionStatus.state === next.state
    && reportedConnectionStatus.detail === next.detail
  ) {
    return;
  }
  reportedConnectionStatus = next;
  try {
    await chrome.storage.local.set({ [CONNECTION_STATUS_KEY]: next });
  } catch (_error) {
    // 状态上报是尽力而为，不能因此中断命令轮询。
  }
}
...
// 整页采集覆盖 1688 与淘宝/天猫列表页；淘宝走同一套 prepare/start/item/finish 链路。
...
async function startConnection() {
  ...
  await ensureOpenWorkbenchPageControls();
  // 旧实现无论轮询结果如何都返回 { ok: true }，popup 因此永远显示“已连接”。
  // 现在只有工作台真正接受了这次会话（首轮轮询成功）才回报成功。
  const accepted = await pollOnce(connection);
  if (accepted) {
    return { ok: true };
  }
  const stored = await chrome.storage.local.get(CONNECTION_STATUS_KEY);
  const detail = stored?.[CONNECTION_STATUS_KEY]?.detail;
  if (connectionMatchesActive(connection)) {
    await clearConnectionState();
  }
  return {
    ok: false,
    error: "plugin session not accepted by workbench",
    statusText: detail || ""
  };
}
...
// WebSocket 是命令下发的实时通道（最坏情况由 1 分钟 alarm + 轮询兜底）。
// 旧实现 onclose 只把 socket 置空就再也不重连，浏览器回收连接、后端重启、
// 网络抖动之后实时通道会永久失效。这里按指数退避重连（3s→60s 封顶）。
function scheduleWebSocketReconnect(connectionContext) {
  ...
  const delay = Math.min(
    WS_RECONNECT_BASE_DELAY_MS * (2 ** Math.min(wsReconnectAttempts, 4)),
...
function connectWebSocket(connectionContext) {
  try {
    ...
    currentSocket.onopen = () => {
      ...
      wsReconnectAttempts = 0;
      void recordConnectionStatus("connected");
    };
    ...
    currentSocket.onclose = () => {
      if (socket !== currentSocket) return;
      socket = null;
      scheduleWebSocketReconnect(connection);
    };
  } catch (error) {
  ...
  try {
    ...
    if (!response.ok) {
      if ([401, 403, 404].includes(response.status)) {
        // 工作台已拒绝该插件会话（后端重启或 10 分钟无活动过期都会这样）。
        // 旧实现静默清空本地状态，popup 仍显示“已连接”，用户看不到真实原因。
        await recordConnectionStatus(
          "session_rejected",
          `工作台拒绝了插件会话（HTTP ${response.status}），请重新登录工作台后点“连接插件”`
        );
        await clearConnectionState();
      } else {
        await recordConnectionStatus("unreachable", `工作台返回 HTTP ${response.status}`);
      }
      return false;
    }
    const payload = await response.json();
    if (!connectionMatchesActive(connection)) return false;
    await recordConnectionStatus("connected");
    ...
    if (error instanceof tenantContext.TenantContextError && connectionMatchesActive(connection)) {
      await recordConnectionStatus(
        "session_rejected",
        "工作台返回的租户上下文与本地会话不一致，请重新登录工作台后点“连接插件”"
      );
      await clearConnectionState();
    }
